...
public sealed partial class PumpControl : BoxContainer
{
    ...
    private GridContainer _gases => CGasContainer; //Ratbite - gas lockout
    private Dictionary<Gas, Button> _gasControls = new(); //Ratbite - gas lockout

    private ThresholdControl _temperatureThreshold; //Ratbite - Pressure lockout
    ...
    public PumpControl(GasVentPumpData data, string address)
    {
        ...
        foreach (var value in Enum.GetValues<Gas>())
        {
            var gasButton = new Button
            {
                ...
                Pressed = _data.GasLockoutGases.Contains(value)
            };
            gasButton.OnToggled += args =>
            {
                if (args.Pressed)
                    _data.GasLockoutGases.Add(value);
                else
                    _data.GasLockoutGases.Remove(value);
        ...
        //End of Ratbite Gas Lockout

        _temperatureThreshold = new ThresholdControl(Loc.GetString("air-alarm-ui-thresholds-temperature-title"),
            data.TemperatureLockoutThreshold,
            AtmosMonitorThresholdType.IncomingTemperature);
        _temperatureThreshold.CWarningBounds.Visible = false;
    ...
    public void ChangeData(GasVentPumpData data)
    {
        ...
        foreach (var value in Enum.GetValues<Gas>())
        {
            _gasControls[value].Pressed = data.GasLockoutGases.Contains(value);
        }

        _temperatureThreshold.UpdateThresholdData(data.TemperatureLockoutThreshold, data.Temperature);
    }

    // Assmos changes start here
    private void SelectAllGases()
    {
        _data.GasLockoutGases.Clear();
        foreach (var (gas, button) in _gasControls)
        {
            button.Pressed = true;
            _data.GasLockoutGases.Add(gas);
        }
        PumpDataChanged?.Invoke(_address, _data);
    }
